...
| RT companion-event construction | Established for source; production audit pending | Among 6,655 responded trials, 931 (13.99%) lack a companion `event_RT` row. All 805 responded first trials of blocks lack that row; the other 126 omissions are every non-first trial in both runs for sub-143, whose files contain no `event_RT` rows. The two tracked sub-143 event TSVs are byte-identical to OpenNeuro ds003745 snapshot 2.0.2 (commit `88d8d458`) and each retains all 72 substantive response rows. The production root `/ZPOOL/data/projects/srndna-ultimatum` retains sub-143 activation, DMN, and ECN L2 copes 4, 6, and 7 from August 2021. The retained L1 designs/EVs and submitted L3 directory must be audited there before proposing any regeneration. Counts previously obtained from `srndna-ug` are not production evidence. |
